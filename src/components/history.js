...
function History() {
    ...
    useEffect(() => {
        if (localStorage.getItem("email") == null && window.location.pathname === "/history") {
            navigate("/");
        }
        if (localStorage.getItem("data")) {
            const data = Object.entries(JSON.parse(localStorage.getItem("data"))).sort((a, b) => {
                return parseInt(b[1].date.split("-").join("")) - parseInt(a[1].date.split("-").join(""));
            });
    ...
    const handleRemove = async (entryId) => {
        ...
        try {
            ...
            setProcessedData((prevData) => {
                ...
                localStorage.setItem("data", JSON.stringify(Object.fromEntries(updatedData)));
                return updatedData;
            });
        } catch (error) {
