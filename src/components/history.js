...
function History() {
    ...
    if (localStorage.getItem('data')) {
        dataProccessed = Object.entries(JSON.parse(localStorage.getItem('data')))
            .sort((a, b) => {
                return parseInt(b[1].date.split('-').join(''))
                            ...
                            <tbody id={dataProccessed[index][1].date}>
                                {
                                    dataProccessed.map(
                                        // eslint-disable-next-line no-loop-func, array-callback-return
                                        (childs) => {
                                            if (childs[1].date.substr(0, 7) === dataProccessed[index][1].date.substr(0, 7)) {
                                                ind++;
                                                return (<tr id={childs[0]} key={childs[0]}>
                                                    <td>{childs[1].cash}</td>
                                                    <td>{childs[1].note}</td>
